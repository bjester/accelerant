...
describe('FirestorePath', () => {
  it('provides compatibility path helpers', () => {
    const docPath = new FirestorePath(new Request('https://example.com/api/db/users/u1'), '/api/db');
    expect(docPath.isDocumentPath).to.equal(true);
    expect(docPath.getCollectionPath()).to.equal('/users');

    const subCollectionPath = new FirestorePath(new Request('https://example.com/api/db/users/u1/items'), '/api/db');
    expect(subCollectionPath.isSubCollectionPath).to.equal(true);
    expect(subCollectionPath.getFirestoreCollectionGroupPath().getCollectionPath()).to.equal('/items');
  });
});
